...
contract AntfarmAtfPair is IAntfarmAtfPair, ReentrancyGuard, Math {
    ...
    // DIVIDEND VARIABLES
    uint256 private totalDividendPoints = 0;
    uint256 private unclaimedDividends = 0;
        ...
        returns (uint256, uint256)
    {
        ...
        positions[msg.sender][positionId].lp =
            positions[msg.sender][positionId].lp -
            uint128(liquidity);
    ...
    function setOracleInstance() internal {
        antfarmOracle = new AntfarmOracle(
            token1,
    ...
    function _disburse(uint256 amount) private {
        totalDividendPoints +=
            (amount * POINT_MULTIPLIER) /
            (totalSupply - MINIMUM_LIQUIDITY);
        unclaimedDividends += amount;
        antfarmTokenReserve += amount;
    }
}
